            ...
            for block in range(Num_block):
                for H in range(16):
                    for W in range(16):
                        for chn in range(32):
                            tmp_array = [0, 2, 2]
                            tmp = tmp_array[ (chn + 32*W) % 3]
                            # tmp = abs( round(random.gauss(0, 0.6)) ) # pool1
                            if abs(tmp) >= theshold :
                                cnt_element += 1
                                flag = 1
                    ...
                    for H in range(shape[3]):
                        for W in range(shape[4]):
                            ...
                            for chn in range(shape[1]):
                                ...
                                tmp_array = [0, 2, 2]
                                tmp = tmp_array[ (chn + 32*W) % 3]
                                # tmp = abs( round(random.normalvariate(-0.137, 0.6)) )# conv2
                                if tmp != 0 :
                                    cnt_element += 1
                                    cnt_sparsity += 1
                                    addr_element += 1
...
random.seed(000)
net = 'conv2'
tensor = torch.ones([8,  64, 4,  14,  14])
scale = 1
